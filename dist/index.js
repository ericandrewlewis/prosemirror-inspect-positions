...
var DOMParseState = function () {
  ...
  _createClass(DOMParseState, [{
    ...
    value: function parseNodeType(dom) {
      ...
      var sync = void 0,
          before = void 0;
      if (result.node && result.node.isLeaf) this.insertNode(result.node.create(result.attrs));else if (result.node) sync = this.enter(result.node, result.attrs);else before = this.addMark(result.mark.create(result.attrs));
...
var _require5 = require('./position-helpers');

var renderDocumentPositions = _require5.renderDocumentPositions;
var renderUserPosition = _require5.renderUserPosition;
...
renderDocumentPositions(pm.doc);
renderUserPosition(pm.selection);
pm.on.transform.add(function (transform) {
  renderDocumentPositions(pm.doc);
});
pm.on.selectionChange.add(function (transform) {
  renderUserPosition(pm.selection);
});
...
},{"./position-helpers":67,"prosemirror/dist/edit":10,"prosemirror/dist/example-setup":20,"prosemirror/dist/menu":29,"prosemirror/dist/schema-basic":44}],67:[function(require,module,exports){
'use strict';

var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol ? "symbol" : typeof obj; };

var getNodePositions = function getNodePositions(node) {
  if (node.type.isLeaf) {
    if (node.type.isText) {
    ...
    var _ret = function () {
      var positions = ['start ' + node.type.name];
...
var renderDocumentPositions = function renderDocumentPositions(doc) {
  var positions = [];
  doc.forEach(function (node) {
    positions = positions.concat(getNodePositions(node));
  });
  var table = document.querySelector('#positionInfo');
  table.innerHTML = '';
  var positionIndexesRow = document.createElement('tr');
  var positionContentRow = document.createElement('tr');
...
var renderUserPosition = function renderUserPosition(selection) {
  var positions = [];
  var dom = document.querySelector('#cursorPositionInfo');
  dom.innerHTML = 'Current position: From ' + selection.from + ' to ' + selection.to;
};

module.exports = {
  renderDocumentPositions: renderDocumentPositions,
